...
int main()
{
    Board board(Bldc::Param::CAN_BASE_ID);
    ...
    printf("BLDCDriver V3.0 :: CAN ID = %d\n", board.can_id);
    
    EPBFeedBack::Can epb_decoder;
    ...
    while (!DEBUG_MODE)
    {
        ...
        while (GlobalInterface::can_buff.get(rcv_msg)){
            ...
            if (Bldc::Can::isBoardCanID(rcv_msg, board.can_id)){
    ...
    {           
        ...
        if(board.buttons[0]->getState())
            motor_test.update(board.can_id, p1, 0.5f);
        else
            motor_test.update(board.can_id, p1, 0.f);
    ...
    uint16_t can_id = Bldc::Param::CAN_BASE_ID;
    ...
    while (1)
    {
        ...
        while (GlobalInterface::can_buff.get(rcv_msg)){
            board.leds[1]->flash(6);
            if (Bldc::Can::isBoardCanID(rcv_msg, can_id)){
                board.leds[2]->flash(6);
                motor_control.setControl(rcv_msg);
            } else{
                printf("                    ElseID: %d\n", rcv_msg.id);
