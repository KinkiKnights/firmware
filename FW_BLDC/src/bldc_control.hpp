...
public:
    ...
    void setControl(CanMessage& msg){
        wd_counter = 0;
        decoder.decode(msg);
        printf("getCAN %f, %f\n", decoder.getDuty(0), decoder.getDuty(1));
...
public:
    ...
    void update(uint8_t child_id, CanMessage& msg, float duty){
        if(buttons[0]->getState())
            encoder.setDuty(test_duty,0);
        else 
            encoder.setDuty(0.f,0);
        if(buttons[1]->getState())
            encoder.setDuty(test_duty,1);
        else 
            encoder.setDuty(0.f,1);
