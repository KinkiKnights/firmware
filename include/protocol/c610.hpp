...
namespace DEF
    ...
    public: // ユーティリティ関数
        ...
        static uint8_t getChildNumber(CanMessage& msg){
            ...
            if (id < Param::BOARD_NUM) return id -1;
            else return 0xff;
        }
    ...
    public: // メッセージエンコード・デコード
        ...
        CanMessage encode(uint16_t child_id, uint8_t port = 0){
            ...
            for(uint8_t port = 0; port < 4; port++){
                ...
                section[0] = (speed[port] >> 8) & 0xFF;
                section[1] = speed[port] & 0xFF;
            }
            return msg;
        }
        ...
        void decode(CanMessage& msg){
            for(uint8_t port = 0; port < 2; port++){
                uint8_t* section = &msg.data[port*2];
                speed[port] = section[0];
                speed[port] = (speed[port]<<8) + section[1];
            }
        }
    public: // 応用関数
...
namespace C610_FB{
    namespace DEF
        {
            constexpr uint32_t MAX_TARGET = 10000;
        } // namespace name
        ...
        public: // 設定値
            ...
            uint8_t child_id = 0;
            // 現在位置
            uint16_t position[Param::PORT_NUM];
            uint16_t speed[Param::PORT_NUM];
            uint16_t torque[Param::PORT_NUM];
        ...
        public:
            ...
            uint8_t encode(uint8_t* frame){
                // ヘッダ情報の設定
                uint8_t dlc = 6 * Param::PORT_NUM; 
                frame[0] = Param::SERIAL_ID;
                frame[1] = dlc;
                for(uint8_t idx = 0; idx < Param::PORT_NUM; idx++){
                    // フレームの設定
                    uint8_t* section = &frame[idx*6 + 3];
                    section[0] = position[idx] & 0xFF;
                    section[1] = (position[idx] >> 8) & 0xFF;
                    section[2] = speed[idx] & 0xFF;
                    section[3] = (speed[idx] >> 8) & 0xFF;
                    section[4] = torque[idx] & 0xFF;
                    section[5] = (torque[idx] >> 8) & 0xFF;
            ...
            void decode(uint8_t* frame){
                ...
                for(uint8_t idx = 0; idx < 2; idx++){
                    uint8_t* section = &frame[idx*3 + 3];
                    position[idx] = section[0] + ((uint16_t)section[1] << 8);
                    speed[idx] = section[2] + ((uint16_t)section[3] << 8);
                    torque[idx] = section[4] + ((uint16_t)section[5] << 8);
                }
            }
        };
};