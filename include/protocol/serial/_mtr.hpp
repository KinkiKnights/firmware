...
namespace MotorFeedBack{
    struct Serial{
        ...
        static const uint8_t PORT_MAX = 2; // 最大送信ポート数
        static const uint8_t PORT_NUM = 2; // 基板一つあたりのポート数
        static const uint8_t PORT_BLOCK = 6; // 1ポート情報当たりのバイト数
        // ポート番号から、該当する基板IDを算出します。
        static uint8_t getBoardID(uint8_t port){
            return port / PORT_NUM;
        }
    public: // 設定値
        ...
        // フィードバック対象ポート
        uint8_t child_id = 0;
        int16_t fb_output[PORT_MAX];
        int32_t fb_encoder[PORT_MAX];
    public:
        /**
         * @brief シリアルコマンドのエンコード
         * @note farme1のDLCは、フレーム2以降のフレーム数、帰り値は総フレーム数
        ...
        uint8_t encode(uint8_t* frame){
            ...
            for (uint8_t idx = 0; idx < port_num; idx++){
                ...
                uint8_t* section = &frame[2 + idx * PORT_BLOCK];
                // ターゲットを設定
                section[0] = static_cast<uint8_t>((fb_encoder[idx] >> 24) & 0xFF);
                section[1] = static_cast<uint8_t>((fb_encoder[idx] >> 16) & 0xFF);
                section[2] = static_cast<uint8_t>((fb_encoder[idx] >> 8) & 0xFF);
                section[3] = static_cast<uint8_t>(fb_encoder[idx] & 0xFF);
                section[4] = static_cast<uint8_t>((fb_output[idx] >> 8) & 0xFF);
                section[5] = static_cast<uint8_t>(fb_output[idx] & 0xFF);
                frame[1] += PORT_BLOCK;
            }
            // 全体長(DLC + 2)を返す
            return frame[1] + 2;
        ...
        bool decode(uint8_t* frame){
            ...
            for (uint8_t idx = 0; idx < port_num; idx++){
                ...
                fb_encoder[idx] = static_cast<int32_t>(
                    section[3] | (section[2] << 8) | (section[1] << 16) | (section[0] << 23)
                );
                fb_output[idx] = static_cast<int16_t>(
                    section[1] | (section[0] << 8)
                );
            }
        }
    };
