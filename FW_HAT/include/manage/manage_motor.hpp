...
namespace BoardManager
{
    ...
    class MotorManager{
        ...
        // コントロールはCAN側に基板実態を格納
        Motor::Serial serial_decoder;
        Motor::Can* control_model[CONTROL_UNIT_MAX];
        BoardLifeCycle board_life[CONTROL_UNIT_MAX];
        BoardLifeCycleH control_life[CONTROL_UNIT_MAX * 2];
        // 返り値はシリアル側に基板実態を格納
        // EncoderFeedback::Can can_decoder;
    ...
    public: // インターフェイス関数群
        MotorManager(){
            ...
            for (uint8_t child_idx = 0; child_idx < CONTROL_UNIT_MAX; child_idx++ ){
                control_model[child_idx] = new Motor::Can(child_idx);
            }
    ...
    public:
        ...
        void update(uint16_t update_ms){
            ...
            for (uint8_t board_idx = 0; board_idx < CONTROL_UNIT_MAX; board_idx++){
                // 制御ポートのタイムオーバー
                for (uint8_t port_idx = 0; port_idx <CONTROL_UNIT_PORT; port_idx++){
                    if(!control_life[board_idx * 2 + port_idx].update(update_ms))
                        control_model[board_idx]->target[port_idx] = 0;
                }
        ...
        bool rcvCommand(uint8_t* msg){
            // コマンド種別を判別して無効なら処理終了
            if (msg[0] != Motor::Serial::SERIAL_ID) return false;
            ...
            for (uint8_t idx = 0; idx < serial_decoder.port_num; idx++){
                ...
                // ライフサイクルの更新
                control_life[port].set();
                // 制御情報の格納
                control_model[ctrl_idx]->target[port_idx] = serial_decoder.duty[idx];
                printf("%d=>%d, ", port, serial_decoder.duty[idx]);
            }
            printf("\n");
            return true;
        }
        ...
        bool rcvLive(Live::Can& decode_msg){
            if (decode_msg.board_id < Motor::BASE_CAN_ID) return false;
            if (decode_msg.board_id > (Motor::BASE_CAN_ID + 0xF)) return false;
            board_life[decode_msg.board_id - Motor::BASE_CAN_ID].set();
            return true;
        }
    };
}