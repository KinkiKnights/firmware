...
public:
    void update(uint16_t term_ms){
        for (uint8_t port = 0; port < 8; port++){
            ...
            // カウンタピリオド設定
            pwms[port]->setPeriod((pos_duty_current[port] -1000)*2);
        }
    }

    ServoMessage decoder;
    void setControl(CanMessage& msg){    
        ...
        if (decoder.offset_port)
            port_offset = 4;
        // デコード結果格納
        for (uint8_t port_idx = 0; port_idx < 4; port_idx++){
            pos_duty_target[port_idx + port_offset] = decoder.counter[port_idx];
...
class PwmServoTest{
    ...
    const uint16_t MIN_COUNT = 1000;
    const uint16_t PLS_DIFF = 5;

    ServoMessage* encoders[2];
    Button** buttons;
...
public:
    PwmServoTest(Button** _buttons){
        buttons = _buttons;
        encoders[0] = new ServoMessage();
        encoders[1] = new ServoMessage();
    ...
    void genMsg(uint16_t can_id, CanMessage& p1, CanMessage& p2){
        ...
        // 値更新
        for (uint8_t idx = 0; idx < 4; idx++){
            encoders[0]->counter[idx] = target_count;
            encoders[1]->counter[idx] = target_count;
